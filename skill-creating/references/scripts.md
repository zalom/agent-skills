...
A skill validates that a skill name slug is well formed before any directory is created.
The check runs on every skill and must be exact, so it ships as a script, not prose.
[G1][G3]
...
```ruby
#!/usr/bin/env ruby
# Validate a skill name slug. Execute this; do not inline its logic.
...
skill well") would stay prose; this fixed check is a script.
