...
(pass^k) with k at least three. The `skill-evaluating` skill owns the grader taxonomy, judge
calibration, clean-context execution, and cross-model testing. [F6, F7, F8]

## Run them with the methodology skill

Use the `skill-evaluating` skill to author and run the evals. It owns the full method: eval scope
