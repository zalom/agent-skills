...
- Give the script a shebang (`#!/usr/bin/env ruby` for a Ruby handler; state the shell and
  minimum version a shell handler targets). [E3]
...
- To block (a gate that refuses an edit, a validator that rejects a command), exit 2 and write
  the reason to stderr. The agent reads stderr and can act on it, for example:
  `$stderr.puts "GATE - <reason>"; exit 2`.
