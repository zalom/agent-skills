...
- **henitai 0.5.3** declares Ruby 3.3.6 or later (it does not install on 3.3.5); every
  measured run used Ruby 4.0.3, and it was never run on 3.3.x. On Ruby 4.0.3 it is a working
  fallback on Minitest 6 and RSpec 3.13: its survivors covered the same 2 known gaps as
  Mutineer on the same fixture. Its RSpec integration needs `require "simplecov"` and
  `SimpleCov.start` written by hand in `spec/spec_helper.rb`, or the run stops with
  `Henitai::CoverageError` (its Minitest integration auto-requires that bootstrap; RSpec's
  does not). 10 of 54 mutants (18.5%) needed a flaky retry in the measured sample. Run it with
  `bundle exec henitai run --operators full` (Minitest, auto-detected) or `--use rspec` for
  RSpec.
